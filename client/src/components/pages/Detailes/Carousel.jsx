  ...
  useEffect(() => {
    ...
    buttons.forEach((button, idx) => {
      button.addEventListener("click", (e) => {
        ...
        if (idx == 1 && slides[slides.length - 1].style.top == "390px")
          buttons[1].style.display = "none";
        else buttons[1].style.display = "block";
        if (idx == 0 && slides[0].style.top == "0px")
        ...
        <List className="text-center" position="relative">
          {carouselImg.map((img, idx) => {
            return (
              <ListItem
                key={idx}
                onMouseEnter={() => setUrl({ src: img.url, isImg: true })}
              >
                <Box>
          ...
          <ListItem
            key={1}
            onMouseEnter={() => setUrl({ src: v1, isImg: false })}
          >
            <Box>
              <video className="d-block w-100 h-100" controls autoPlay loop>
                <source src={v1} className="d-block w-100 h-100" />
              </video>
            </Box>
          </ListItem>
